...
nconf.defaults({
  'GRAPHICBUILDER_KIT_GOOGLE_CLIENT_SECRETS': '~/.graphicbuilder_kit_google_client_secrets.json',
  'GRAPHICBUILDER_KIT_TOKEN': '~/.graphicbuilder_kit_token.json'
});

var secretFile = untildify(nconf.get('GRAPHICBUILDER_KIT_GOOGLE_CLIENT_SECRETS'));
...
function primeToken(client, cb) {
  fs.readFile(untildify(nconf.get('GRAPHICBUILDER_KIT_TOKEN')), 'utf8', function(err, data) {
...
function buildDataJSON(data) {
  ...
  fs.writeFileSync('app/assets/campus-results.json', JSON.stringify(payload.RANKINGS));
}
